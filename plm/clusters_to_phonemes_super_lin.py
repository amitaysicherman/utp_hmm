...
if __name__ == '__main__':
    ...
    train_data = DataLoader(CustomDataset(f"data/LIBRISPEECH_TRAIN_clusters_{input_size}.txt", "data/LIBRISPEECH_TRAIN_letters.txt"),
                            batch_size=64, shuffle=True, drop_last=True)
    test_data = DataLoader(CustomDataset(f"data/LIBRISPEECH_TEST_clusters_{input_size}.txt", "data/LIBRISPEECH_TEST_letters.txt"),
                           batch_size=64, shuffle=True, drop_last=True)
    ...
    for epoch in range(num_epochs):
        ...
        if test_acc > best_test_acc:
            ...
            torch.save(model.state_dict(), f"models/simple_seq2seq_model_best_letters_{input_size}.pth")
            mapping = torch.arange(input_size).to(device).unsqueeze(0)
            mapping = model(mapping)[0].cpu().detach().numpy().argmax(axis=-1)
            with open(f"models/clusters_phonemes_map_letters_{input_size}.txt", "w") as f:
                f.write("\n".join([str(x) for x in mapping]))

    # Save the trained model
    torch.save(model.state_dict(), f"models/simple_seq2seq_model_letters_{input_size}.pth")
